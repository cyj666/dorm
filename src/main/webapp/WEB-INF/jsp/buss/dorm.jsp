...
<body>
	<table id="dg" class="" title="宿舍管理" style="width: 100%; height: 100%"
		...
		<thead>
			<tr>
				...
				<th data-options="field:'roomStatus',width:15"
					formatter="manageStatus">房间状态</th>
				...
				<th data-options="field:'employees',width:10"
					formatter="manageEmployee">已住人数</th>
	...
	<div id="toolbar">
		<a href="#" class="easyui-linkbutton add" iconCls="icon-add"
			onclick="create()" plain="true">入住登记</a>
		...
		<a href="#" class="easyui-linkbutton remove" iconCls="icon-remove"
			onclick="remove1()" plain="true">搬离宿舍</a>
		<!--<<a href="#" class="easyui-linkbutton remove" iconCls="icon-cancel" onclick="del()" plain="true">删除记录</a>-->
		<a href="#" class="easyui-linkbutton put" iconCls="icon-undo"
			onclick="put()" plain="true">导入</a> <a href="#"
			class="easyui-linkbutton export" iconCls="icon-redo"
			onclick="print()" plain="true">导出</a>
		<div>
			...
			<a href="#" class="easyui-linkbutton" onclick="doSearch()"
				...
				onclick="doReset()" iconCls="icon-redo">重置</a>
		</div>
	</div>
	<div id="exceldlg" class="easyui-dialog" title="导入excel文件"
		...
		<form id="uploadExcel" method="post" enctype="multipart/form-data">
			选择文件： <input id="excel" name="excel" class="easyui-filebox"
	...
	<div id="dlg" class="easyui-dialog" title="数据参数"
		data-options="modal:true" style="width: 50%; height: 70%; top: 0px"
		...
		<form method="get" id="fm">
			<table cellpadding="9">
				...
				<tr>
					<td>总床位数:</td>
					<td><input class="easyui-textbox" id="size" type="text"
						name="size" maxlength="2" validtype="number" required="true"
						readonly="readonly" disabled="disabled"></input></td>
				</tr>
				<tr>
					<td>类型:</td>
				...
				<tr>
					...
					<td><input class="easyui-textbox" required="true"
						maxlength="10" type="text" name="roomAdmin" readonly="readonly"
						disabled="disabled"></input></td>
				</tr>
				<tr>
					<td>已住人数:</td>
				...
				<tr>
					<td><input class="easyui-textbox" type="hidden"
						name="employeeId" id="employeeId2" /></td>
				</tr>
				<tr>
					<td>目标员工</td>
					<td><input class="easyui-textbox" readonly="readonly"
						id="employeeNo2" type="text" name="employeeNo" maxlength="10"
						validtype="employeeNo" required="true" missingMessage="工号必须填写"></input>
						<a href="#" class="easyui-linkbutton" iconCls="icon-search"
						onclick="openWin('dlgEmployee','员工')">选择</a> <a href="#"
						class="easyui-linkbutton" iconCls="icon-redo"
						onclick="clearText('classid','classname');">清空</a></td>
				</tr>
				<tr>
					<td>入住/搬出时间</td>
					<td><input class="easyui-datebox"
						data-options="formatter:myformatter,parser:myparser"
						id="employeeWorkplace" maxlength="15" type="text" name="dateIn"></input></td>
				</tr>
			</table>
		</form>
	</div>
	...
	<!-- 员工界面 -->
	<div id="dlgEmployee" class="easyui-dialog" title="数据"
		style="width: 70%; height: 100%; top: 0px;" data-options="modal:true"
		closed="true" buttons="#dlgEmployee-buttons">
		<table id="dgEmployee" class="easyui-datagrid" cellpadding="0"
			...
			<thead>
				<tr>
					...
					<th data-options="field:'employeeStatus',width:80"
						formatter="manageStatus1">状态</th>
	...
	<div id="employee-toolbar">
		<div>
			员工姓名: <input class="easyui-textbox" id="paramEmployee"> <br />
			<a href="#" class="easyui-linkbutton" onclick="doSearchEmployee()"
				iconCls="icon-search">查找</a> <a href="#" class="easyui-linkbutton"
				onclick="doResetEmployee()" iconCls="icon-redo">重置</a>
		</div>
	</div>
	<div id="dlgEmployee-buttons">
		<a href="#" class="easyui-linkbutton  save"
			onclick="afterSelect('dgEmployee','dlgEmployee','employeeid','employeeNo')"
			iconCls="icon-ok">确定</a> <a href="#" class="easyui-linkbutton cancel"
			onclick="javascript:$('#dlgEmployee').dialog('close')"
	...
	<script type="text/javascript">
		...
		function doSearchEmployee() {
			var queryCondition = {
				'employeeName' : $('#paramEmployee').val()
			};
		...
		function create() {
			...
			if (row.length == 1) {
				if (row[0].employees.length < row[0].size) {
					...
					$.messager.alert('提示信息', row[0].roomNo + '寝室已经住满!',
							'warning');
				}
	...
	<script type="text/javascript">
		...
		$("#uploadExcel").form({
			type : 'post',
			url : '${pageContext.request.contextPath}/putExcel3',
			dataType : "json",
			onSubmit : function() {
		...
		function print() {
			window.location.href = "${pageContext.request.contextPath}/printExcel3";
		}
	</script>
</body>
</html>